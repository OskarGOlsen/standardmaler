...
df <- df %>% 
  mutate(ny_delomr = substr(newCode, 1, 6),
         gammel_delomr = substr(oldCode, 1, 6),
         endringsaar = substr(changeOccurred, 1, 4)) %>% 
  # Gir nye variabelnavn
  distinct(ny_delomr, gammel_delomr, endringsaar) 
...
df <- df %>% 
  filter(gammel_delomr != ny_delomr)
...
sjekk <- setNames(df$ny_delomr, df$gammel_delomr)
...
delomr_endr <- df %>% 
  rowwise() %>% 
  mutate(siste = siste(ny_delomr)) %>% 
  distinct(gammel_delomr, siste)
